...
import { TpoPreviousContactsView } from '@/components/ui/TpoPreviousContactsView';
...
export default function TpoPortalPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        const savedState = sessionStorage.getItem('tpoPortalState');
        if (savedState) {
          const parsed = JSON.parse(savedState);
          if (parsed.selectedTPO !== undefined) setSelectedTPO(parsed.selectedTPO);
          if (parsed.tpoCategory !== undefined) setTpoCategory(parsed.tpoCategory);
          ...
          if (parsed.showToTPR !== undefined) setShowToTPR(parsed.showToTPR);
          if (parsed.returnToUnified !== undefined) setReturnToUnified(parsed.returnToUnified);
        }
      } catch (e) {
        console.error("Failed to restore tpo portal state", e);
      }
    }
  }, []);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const stateToSave = {
        selectedTPO, tpoCategory, activeView, previousView, dashboardTab, activeCompanyId, lastVisitedCompanyId, activeCategory, listSearchQuery,
        outcome, customOutcome, channel, notes, nextContactDate, showToTPR, returnToUnified
      };
      sessionStorage.setItem('tpoPortalState', JSON.stringify(stateToSave));
    }
  }, [selectedTPO, tpoCategory, activeView, previousView, dashboardTab, activeCompanyId, lastVisitedCompanyId, activeCategory, listSearchQuery, outcome, customOutcome, channel, notes, nextContactDate, showToTPR, returnToUnified]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (tpoName && !selectedTPO && activeTpos) {
        const tpo = activeTpos.find((t: any) => t.name === tpoName);
        if (tpo) {
          setTpoCategory(tpo.type as 'Faculty' | 'Staff');
        }
          ...
          {tpoCategory && (
            <div className="w-full animate-in fade-in slide-in-from-left-4 duration-300">
              ...
              >
                ...
                {tpoCategory === 'Staff' && (
                  staffTpos.map((t: any) => <option key={t._id} value={t.name}>{t.name}</option>)
                )}
                {tpoCategory === 'Faculty' && (
                  facultyTpos.map((t: any) => <option key={t._id} value={t.name}>{t.name}</option>)
                )}
              </select>
            </div>
          )}
                              ...
                              <div className="bg-white p-4 rounded-xl border border-indigo-100 shadow-sm hover:shadow-md transition-shadow relative group">
                                ...
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <p className="font-bold text-slate-900 truncate pr-12 sm:pr-24 text-base min-w-0">{hr.name || 'Unknown Name'}</p>
        ...
        <div className="mt-8 space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <button 
              ...
              className="text-sm font-medium text-slate-500 hover:text-slate-700 bg-white border border-slate-200 px-4 py-2.5 rounded-xl w-full sm:w-auto text-center shadow-sm"
            >
              Back to Dashboard
            </button>
          </div>
            ...
            return (
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className={`p-5 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${
                  ...
                  <h3 className={`text-xl font-bold flex flex-wrap items-center gap-2.5 ${
                    activeCategory === 'not_contacted' ? 'text-slate-800' :
                    activeCategory === 'call_again' ? 'text-blue-900' :
                    'text-amber-900'
                  }`}>
                    {activeCategory === 'not_contacted' && <Users className="w-6 h-6 text-slate-500 shrink-0" />}
                    {activeCategory === 'call_again' && <Calendar className="w-6 h-6 text-blue-600 shrink-0" />}
                    {activeCategory === 'pending' && <Clock className="w-6 h-6 text-amber-600 shrink-0" />}
                    <span>{title} Queue</span>
                    <span className={`text-xs font-bold px-3 py-1 rounded-full border bg-white shadow-sm shrink-0 whitespace-nowrap ${
