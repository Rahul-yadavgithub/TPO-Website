...
export function TpoPreviousContactsView({ tpoName, tpoType, onBack }: TpoTpoPreviousContactsViewProps) {
  ...
  const { data: myRequestedData, isLoading: myRequestedLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const cursorParam = cursor ? `&cursor=${encodeURIComponent(cursor)}` : '';
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/previous-companies/list?status=my_requests&page=${myRequestedPage}&limit=10&tpoName=${tpoName}&subTab=${myRequestsSubTab}${cursorParam}`, { withCredentials: true });
      if (res.data?.pagination?.nextCursor) {
  ...
  const { data: othersRequestedData, isLoading: othersRequestedLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const cursorParam = cursor ? `&cursor=${encodeURIComponent(cursor)}` : '';
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/previous-companies/list?status=others_requests&page=${othersRequestedPage}&limit=10&tpoName=${tpoName}${cursorParam}`, { withCredentials: true });
      if (res.data?.pagination?.nextCursor) {
