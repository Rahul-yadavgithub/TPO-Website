...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    // Prioritize the actual origin of the request (production) over the hardcoded local env variable
    const baseUrl = clientOrigin && clientOrigin !== 'null' ? clientOrigin : (process.env.FRONTEND_URL || 'http://localhost:3000');
